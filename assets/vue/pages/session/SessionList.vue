    ...
    <div class="card shadow-sm">
      <div class="card-body p-0">
          ...
          <table class="table table-hover mb-0 align-middle">
            ...
            <tr v-for="session in filteredSessions" :key="session.id"
              ...
              <td class="text-center">
                <a :href="$routing.generate('app_session_show', { id: session.id })" class="btn btn-outline-info btn-sm me-2" title="Voir">
                  <i class="fas fa-eye"></i>
                </a>
                <a :href="$routing.generate('app_session_edit', { id: session.id })" class="btn btn-outline-warning btn-sm me-2" title="Modifier">
        ...
        <div v-else class="p-5 text-center text-muted">
          <img src="https://via.placeholder.com/150?text=No+Data" alt="Aucune session" class="img-fluid mb-3" style="max-width:150px">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-danger btn-sm" @click="deleteSession" data-bs-dismiss="modal">Confirmer</button>
          </div>
        </div>
      </div>
    </div>
  ...
  computed: {
    filteredSessions() {
      return this.allSessions.filter((session) => {
        const searchMatch =
            this.searchInput === "" ||
            session.room.name.toLowerCase().includes(this.searchInput.toLowerCase()) ||
            session.teacher.lastName.toLowerCase().includes(this.searchInput.toLowerCase()) ||
            session.teacher.firstName.toLowerCase().includes(this.searchInput.toLowerCase()) ||
            session.studyClass.name.toLowerCase().includes(this.searchInput.toLowerCase()) ||
            (session.studyClass.speciality &&
                session.studyClass.speciality.toLowerCase().includes(this.searchInput.toLowerCase()));
        ...
        const classMatch =
            ...
            session.studyClass.id === parseInt(this.selectedClassId);
        ...
        const teacherMatch =
            ...
            session.teacher.id === parseInt(this.selectedTeacherId);

        return searchMatch && monthMatch && classMatch && specialityMatch && teacherMatch;
      });
    },
    hasStats() {
  ...
  methods: {
    ...
    isDuplicateSession(session) {
      ...
      return this.filteredSessions.filter(s =>
          ...
          s.teacher.id === session.teacher.id
      ).length > 1;
    }
  },
  mounted() {
